...
1. 通过 [[method/resolvebase/lookup]] 查 `define/<分类模块>/<文件名>.md` 是否已存在。若存在 → 拒绝，返回"文件名重复"。
2. 通过 [[method/resolvebase/lookup]] 查 `<分类模块>` 是否存在。若不存在 → 按 [[method/resolvebase/manage-resolvebase]] 判断是否新建。
3. 通过 [[method/resolvebase/define_grammar_check]] 校验 `文件内容`。不通过 → 拒绝并附违规列表。
4. 创建文件，写入内容。
...
2. 读取当前内容，修改。核心语义变 → 考虑新建而非覆盖。
3. 通过 [[method/resolvebase/define_grammar_check]] 校验修改后内容。不通过 → 拒绝。
4. 写入。
5. 通过 [[method/resolvebase/manage-index]] 更新索引 `description`。
